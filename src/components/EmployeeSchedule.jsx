...
const EmployeeSchedule = () => {
    ...
    const handleEditSchedule = () => {
        if (!editingSchedule)
            return;
        const updatedSchedule = schedule.map(entry => entry.id === editingSchedule.id ? editingSchedule : entry);
    ...
    return (<div className="space-y-6 animate-fade-in">
            ...
            <DialogContent>
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="hourlyRate" type="number" value={newEmployee.hourlyRate} onChange={(e) => setNewEmployee({ ...newEmployee, hourlyRate: parseFloat(e.target.value) })} className="col-span-3"/>
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="contact" className="text-right">Contact</Label>
                  <Input id="contact" value={newEmployee.contact} onChange={(e) => setNewEmployee({ ...newEmployee, contact: e.target.value })} className="col-span-3"/>
                </div>
              </div>
              <DialogFooter>
            ...
            <DialogContent>
              ...
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Select onValueChange={(value) => setNewScheduleEntry({ ...newScheduleEntry, employeeId: value })} value={newScheduleEntry.employeeId}>
                    <SelectTrigger className="col-span-3">
                      <SelectValue placeholder="Select an employee"/>
                    </SelectTrigger>
                    <SelectContent>
                      {employeeList.map(employee => (<SelectItem key={employee.id} value={employee.id}>
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Select onValueChange={(value) => setNewScheduleEntry({ ...newScheduleEntry, day: value })} value={newScheduleEntry.day}>
                    <SelectTrigger className="col-span-3">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="startTime" type="time" value={newScheduleEntry.startTime} onChange={(e) => setNewScheduleEntry({ ...newScheduleEntry, startTime: e.target.value })} className="col-span-3"/>
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="endTime" className="text-right">End Time</Label>
                  <Input id="endTime" type="time" value={newScheduleEntry.endTime} onChange={(e) => setNewScheduleEntry({ ...newScheduleEntry, endTime: e.target.value })} className="col-span-3"/>
                </div>
              </div>
              <DialogFooter>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-8 gap-2 mb-2 font-medium">
                ...
                {daysOfWeek.map(day => (<div key={day} className="text-center">{day.slice(0, 3)}</div>))}
              </div>
              <div className="space-y-4">
                {employeeList.map(employee => (<div key={employee.id} className="grid grid-cols-8 gap-2 items-center">
                    ...
                    {daysOfWeek.map(day => {
                const entry = schedule.find(s => s.employeeId === employee.id && s.day === day);
                return (<div key={day} className="text-center h-12 relative">
                          {entry ? (<div className="bg-primary/10 p-1 rounded text-xs h-full flex flex-col items-center justify-center">
                            ...
                            </div>) : (<div className="border border-dashed border-muted h-full rounded flex items-center justify-center">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {date && (<div>
                ...
                .map(entry => (<div key={entry.id} className="flex justify-between items-center p-2 bg-muted rounded">
                        <div>
                  ...
                  {schedule.filter(entry => entry.day === date.toLocaleDateString('en-US', { weekday: 'long' })).length === 0 && (<p className="text-sm text-muted-foreground">No schedules for this day</p>)}
                </div>
              </div>)}
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {employeeList.map(employee => (<div key={employee.id} className="bg-card border rounded-lg p-4 flex flex-col">
      ...
      {/* Edit Dialog */}
      {editingSchedule && (<Dialog open={!!editingSchedule} onOpenChange={(open) => !open && setEditingSchedule(null)}>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Select onValueChange={(value) => setEditingSchedule({ ...editingSchedule, day: value })} value={editingSchedule.day}>
                  <SelectTrigger className="col-span-3">
                    <SelectValue placeholder="Select a day"/>
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="edit-startTime" type="time" value={editingSchedule.startTime} onChange={(e) => setEditingSchedule({ ...editingSchedule, startTime: e.target.value })} className="col-span-3"/>
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="edit-endTime" className="text-right">End Time</Label>
                <Input id="edit-endTime" type="time" value={editingSchedule.endTime} onChange={(e) => setEditingSchedule({ ...editingSchedule, endTime: e.target.value })} className="col-span-3"/>
              </div>
            </div>
            <DialogFooter>
